...
func (h *Handler) UploadWDB(w http.ResponseWriter, r *http.Request) {
	...
	if wdbHeader.Signature != wdb.SigItem {
		httpapi.Write(ctx, w, http.StatusBadRequest, chroniclesdk.Response{
			Message: fmt.Sprintf("Unsupported WDB signature %q, expected item cache (%q)", wdbHeader.Signature, wdb.SigItem),
		})
		return
	}
	...
	for _, item := range items {
		...
		if !hasReliableDiff {
			unchanged++
			// Still include the diff so the user can see unreliable discrepancies,
			// but don't count as changed or upsert.
		} else {
			changed++
		}
		...
		// mode == "insert" intentionally skips changed items
	}
...
func rowArgs(r database.WorldItemTemplate) []any {
	return []any{
...
func upsertItems(ctx context.Context, pool *pgxpool.Pool, rows []database.WorldItemTemplate) error {
	...
	for i := 0; i < len(rows); i += batchSize {
		...
		for _, r := range rows[i:end] {
			batch.Queue(upsertSQL, rowArgs(r)...)
		}
