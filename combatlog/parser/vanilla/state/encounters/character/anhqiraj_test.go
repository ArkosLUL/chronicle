...
func TestNewCthun_MatchesEyeAndBodyEntries(t *testing.T) {
	...
	for _, entry := range []uint32{15589, 15727} {
		...
		created, ok := character.NewCthun(id, chars)
		require.True(t, ok)
		require.NotNil(t, created)
	}

	created, ok := character.NewCthun(cthunCreatureGUID(16028, 0x99), chars)
	require.False(t, ok)
	require.Nil(t, created)

	created, ok = character.NewCthun(guid.GUID(0x1), chars)
	require.False(t, ok)
	require.Nil(t, created)
}
