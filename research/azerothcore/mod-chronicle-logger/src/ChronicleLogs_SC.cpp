...
//   - OnDamage:               melee auto-attack → SWING (no SpellInfo)
//   - ModifySpellDamageTaken: spell damage       → SPELL_DMG (has SpellInfo)
//   - ModifyHealReceived:     healing            → HEAL (has SpellInfo)
//   - OnAuraApply/Remove:     buff/debuff events
//   - OnUnitDeath:            deaths
// ===========================================================================
...
public:
    ...
    // We use this ONLY for melee auto-attacks (SWING).
    ...
    void OnDamage(Unit* attacker, Unit* victim, uint32& damage) override
    {
        ...
        // Only log melee auto-attacks here.
        // ModifySpellDamageTaken handles spell damage with full SpellInfo.
        // ModifyPeriodicDamageAurasTick handles DoT ticks.
        // We can't perfectly distinguish melee vs spell in OnDamage, but
        // since ModifySpellDamageTaken fires BEFORE OnDamage for spell damage,
        // we use a simple heuristic: always log as SWING here, and accept
        // that spell damage will appear as both SPELL_DMG and SWING.
        //
                            ...
                            SpellInfo const* spellInfo) override
    {
        ...
        uint64 ts = EventFormatter::Now();
        InstanceTracker::Instance().WriteForUnit(
            target, EventFormatter::Heal(ts, target, healer, spellInfo, heal));
    }

    // -----------------------------------------------------------------------
    // OnAuraApply — aura applied → BUFF_ADD or DEBUFF_ADD
    // -----------------------------------------------------------------------
    void OnAuraApply(Unit* unit, Aura* aura) override
    {
        ...
        uint64 ts = EventFormatter::Now();
        InstanceTracker::Instance().WriteForUnit(
            unit, EventFormatter::AuraApply(ts, unit, aura));
    }

    // -----------------------------------------------------------------------
    // OnAuraRemove — aura removed → BUFF_REM or DEBUFF_REM
    // -----------------------------------------------------------------------
    void OnAuraRemove(Unit* unit, AuraApplication* aurApp, AuraRemoveMode /*mode*/) override
    {
        ...
        uint64 ts = EventFormatter::Now();
        InstanceTracker::Instance().WriteForUnit(
            unit, EventFormatter::AuraRemove(ts, unit, aurApp));
    }

    // -----------------------------------------------------------------------
    // OnUnitDeath — unit died → DEATH
    // -----------------------------------------------------------------------
...
// ===========================================================================
// AllSpellScript — captures spell casts (SPELL_GO).
// ===========================================================================
...
public:
                     ...
                     bool /*skipCheck*/) override
    {
        ...
        uint64 ts = EventFormatter::Now();
        InstanceTracker::Instance().WriteForUnit(
            caster, EventFormatter::SpellGo(ts, caster, spellInfo));
    }
};
