...
// Warmane's Spell.dbc has a non-standard layout compared to stock 3.3.5a (12340):
...
//
// We register a patched Spell definition that adds these extra layout columns
// and associates them with a custom build number so the standard 3.3.5a layout
// is not affected (Epoch uses the stock layout).
const WarmaneSpellBuild vsn.Build = 12341 // Warmane-specific pseudo-build for Spell.dbc

func init() {
	SpellBuildOverride = WarmaneSpellBuild

	def, err := dbdefs.Lookup("Spell")
	if err != nil {
		panic("dbcdb: warmane init: " + err.Error())
	}
	...
	if stockLayout == nil {
		panic("dbcdb: warmane init: no 3.3.5a Spell layout found")
	}
	...
	warmaneLayout := dbd.Layout{
		VerifiedBuilds: []vsn.Build{WarmaneSpellBuild},
		Columns:        filtered,
	}

	def.Layouts = append(def.Layouts, warmaneLayout)
	// Re-register with the extra layout appended.
	dbdefs.Register(def)
}
