...
func NewFragmentOfRupturan(id guid.GUID, all *Characters) (Character, bool) {
	entry, ok := id.GetEntry()
	if !ok || entry != fragmentOfRupturanEntry {
		return nil, false
	}

	return NewAdsGoWithBossCustomCharacter(&FragmentOfRupturan{
...
func NewFelheart(id guid.GUID, all *Characters) (Character, bool) {
	entry, ok := id.GetEntry()
	if !ok || entry != 59901 {
		return nil, false
	}
...
func NewRupturanTheBroken(id guid.GUID, all *Characters) (Character, bool) {
	return NewAdsGoWithBoss(rupturanEntry,
		59901, // Felhear
	)(id, all)
}
