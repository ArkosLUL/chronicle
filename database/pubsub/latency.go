...
func (lm *LatencyMeasurer) Measure(ctx context.Context, p Pubsub) (send, recv time.Duration, err error) {
	...
	lm.logger.Debug("performing measurement", slog.String("msg", string(msg)))

	cancel, err := p.Subscribe(lm.latencyChannelName(), func(ctx context.Context, in []byte) {
		if !bytes.Equal(in, msg) {
			lm.logger.Warn("received unexpected message", slog.String("got", string(in)), slog.String("expected", string(msg)))
			return
		}
	...
	select {
	case <-ctx.Done():
		lm.logger.Error("context canceled before message could be received", slog.String("error", ctx.Err().Error()), slog.String("msg", string(msg)))
