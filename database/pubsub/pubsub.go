...
func (p *PGPubsub) subscribeQueue(event string, newQ *msgQueue) (cancel func(), err error) {
	...
	if unlistenInProgress != nil {
		...
		p.logger.Debug("waiting for Unlisten in progress", slog.String("event", event))
		<-unlistenInProgress
		p.logger.Debug("unlistening complete", slog.String("event", event))
	}
	// +-+ (see above)
	defer func() {
		if err != nil {
	...
	if err == nil {
		p.logger.Debug("started listening to event channel", slog.String("event", event))
	}
	...
	return func() {
		...
		func() {
			...
			if !ok {
				p.logger.Error("event was removed before cancel", slog.String("event", event))
				return
			}
			delete(qSet.m, newQ)
			if len(qSet.m) == 0 {
		...
		if unlistening != nil {
			...
			func() {
				...
				qSet, ok := p.queues[event]
				if ok && len(qSet.m) == 0 {
					p.logger.Debug("removing queueSet", slog.String("event", event))
					delete(p.queues, event)
				}
			}()
			...
			if uErr != nil && !p.closedListener {
				p.logger.Warn("failed to unlisten", slog.String("error", uErr.Error()), slog.String("event", event))
			} else {
				p.logger.Debug("stopped listening to event channel", slog.String("event", event))
			}
		}
	}, nil
}
...
func (p *PGPubsub) Publish(event string, message []byte) error {
	p.logger.Debug("publish", slog.String("event", event), slog.Int("message_len", len(message)))
...
func (p *PGPubsub) Close() error {
	p.logger.Info("pubsub is closing")
	err := p.closeListener()
	<-p.listenDone
	p.logger.Debug("pubsub closed")
	return err
}
...
func (p *PGPubsub) listen() {
	defer func() {
		p.logger.Info("pubsub listen stopped receiving notify")
		close(p.listenDone)
	}()
	...
	for notif := range notify {
		...
		if notif == nil {
			p.logger.Debug("notifying subscribers of a reconnection")
			p.recordReconnect()
			continue
		}
...
func (d logDialer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	logger := d.logger.With(slog.String("network", network), slog.String("address", address), slog.Int("timeout_ms", timeoutMS))

	logger.Debug("pubsub dialing postgres")
	start := time.Now()
	conn, err := d.d.DialContext(ctx, network, address)
	if err != nil {
		logger.Error("pubsub failed to dial postgres")
		return nil, err
	}
	elapsed := time.Since(start)
	logger.Debug("pubsub postgres TCP connection established", slog.Int64("elapsed_ms", elapsed.Milliseconds()))
	return conn, nil
}
...
func (p *PGPubsub) startListener(ctx context.Context, connectURL string) error {
	...
	if !ok {
		p.logger.Error("connector does not support setting log dialer, database connection debug logs will be missing")
		panic("connector does not support setting log dialer")
	} else {
		dc.Dialer(dialer)
	}
	...
	p.pgListener = pqListenerShim{
		Listener: pq.NewConnectorListener(connector, connectURL, time.Second, time.Minute, func(t pq.ListenerEventType, err error) {
			...
			case pq.ListenerEventConnected:
				p.logger.Debug("pubsub connected to postgres")
				p.connected.Set(1.0)
			case pq.ListenerEventDisconnected:
				p.logger.Error("pubsub disconnected from postgres", slog.String("error", err.Error()))
				p.connected.Set(0)
			case pq.ListenerEventReconnected:
				p.logger.Info("pubsub reconnected to postgres")
				p.connected.Set(1)
			case pq.ListenerEventConnectionAttemptFailed:
				p.logger.Error("pubsub failed to connect to postgres", slog.String("error", err.Error()))
			}
...
func (p *PGPubsub) Collect(metrics chan<- prometheus.Metric) {
	...
	if err != nil {
		p.logger.Warn("failed to measure latency", slog.String("error", err.Error()))
...
func New(startCtx context.Context, logger *slog.Logger, db *sql.DB, connectURL string) (*PGPubsub, error) {
	...
	go p.listen()
	logger.Debug("pubsub has started")
	return p, nil
}
