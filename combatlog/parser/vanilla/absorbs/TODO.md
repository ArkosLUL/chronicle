# Effective Healing & Absorb Attribution — Vanilla WoW

## Part 1: Effective Healing

Vanilla combat logs don't report overheal. You must infer it by tracking damage taken.

### Algorithm

Maintain a per-unit "HP deficit" counter:

```
On damage taken:  deficit += damage
On heal received: effective = min(heal, deficit)
                  deficit -= effective
                  overheal = heal - effective
```

### Example

| Event | Deficit | Effective | Overheal |
|-------|---------|-----------|----------|
| Takes 5000 dmg | 5000 | — | — |
| Takes 3000 dmg | 8000 | — | — |
| Healed 6000 | 2000 | 6000 | 0 |
| Healed 5000 | 0 | 2000 | 3000 |

### Caveat

The counter runs for the **entire log** — not reset per encounter. It's an approximation.

---

## Part 2: Absorb Attribution

Combat logs say damage was absorbed but not by which spell. Infer it by matching active buffs + damage school.

### Step 1: Absorb Spell Table (Vanilla)

```typescript
// spell_id → [absorbs_schools]
const ABSORB_SPELLS = {
...
### Step 2: Track Buff Uptimes

Parse aura gain/fade messages to build intervals:

```
[timestamp] Player gains Power Word: Shield.
[timestamp] Player's Power Word: Shield fades.
```

Store as:
```
unit_id → spell_id → [(start, end, caster_id), ...]
```

### Step 3: Attribute

When you see absorbed damage:

```python
def attribute_absorb(target, timestamp, damage_school):
    for spell_id, absorbed_schools in ABSORB_SPELLS:
        if not buff_active(target, spell_id, timestamp):
            continue
        if damage_school not in absorbed_schools:
            continue
        return spell_id, get_caster(target, spell_id, timestamp)
    return None
```

School matching lets you distinguish: if a player has both PW:S and Fire Protection active and takes Fire damage, Fire Protection is the more specific match.

---

## Data You Need

1. **Absorb spell table** — ~40 spell IDs for Vanilla
2. **Buff uptime tracking** — Parse gain/fade messages
3. **Damage school** — From the combat log or spell database
4. **Per-unit HP deficit** — Single integer per unit
