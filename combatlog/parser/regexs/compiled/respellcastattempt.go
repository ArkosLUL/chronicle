...
// Code generated by regengo for pattern: (.+[^\s]) begins to (cast|perform) (.+[^\s])\.
// DO NOT EDIT.

var reSpellCastAttemptStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reSpellCastAttemptCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReSpellCastAttempt struct{}

var CompiledReSpellCastAttempt = ReSpellCastAttempt{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReSpellCastAttemptMinMatchLen = 21

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReSpellCastAttemptMaxMatchLen = -1

func (ReSpellCastAttempt) MatchLengthInfo() (minLen, maxLen int) {
	return ReSpellCastAttemptMinMatchLen, ReSpellCastAttemptMaxMatchLen
}

func (ReSpellCastAttempt) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reSpellCastAttemptStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reSpellCastAttemptStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReSpellCastAttempt) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reSpellCastAttemptStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReSpellCastAttemptResult struct {
...
type ReSpellCastAttemptBytesResult struct {
...
func (ReSpellCastAttempt) FindStringReuse(input string, r *ReSpellCastAttemptResult) (*ReSpellCastAttemptResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reSpellCastAttemptCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reSpellCastAttemptCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reSpellCastAttemptStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
Ins34:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 35, 1})
		goto Ins33
	}
Ins35:
	{
...
func (recv ReSpellCastAttempt) FindString(input string) (*ReSpellCastAttemptResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReSpellCastAttempt) FindAllStringAppend(input string, n int, s []*ReSpellCastAttemptResult) []*ReSpellCastAttemptResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins38:
		{
			captures[1] = offset
			var item *ReSpellCastAttemptResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReSpellCastAttemptResult{}
...
func (r ReSpellCastAttempt) FindAllString(input string, n int) []*ReSpellCastAttemptResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReSpellCastAttempt) FindBytesReuse(input []byte, r *ReSpellCastAttemptBytesResult) (*ReSpellCastAttemptBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reSpellCastAttemptCaptureStackPool.Get().(*[]int)
...
Ins38:
	{
		captures[1] = offset
		if r == nil {
			r = &ReSpellCastAttemptBytesResult{}
		}
...
func (recv ReSpellCastAttempt) FindBytes(input []byte) (*ReSpellCastAttemptBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReSpellCastAttempt) FindAllBytesAppend(input []byte, n int, s []*ReSpellCastAttemptBytesResult) []*ReSpellCastAttemptBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins38:
		{
			captures[1] = offset
			var item *ReSpellCastAttemptBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReSpellCastAttemptBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReSpellCastAttemptBytesResult{}
...
func (r ReSpellCastAttempt) FindAllBytes(input []byte, n int) []*ReSpellCastAttemptBytesResult {
...
func (ReSpellCastAttempt) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReSpellCastAttempt) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReSpellCastAttemptBytesResult]) bool) error {
	...
	reuseResult := &ReSpellCastAttemptBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReSpellCastAttempt{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReSpellCastAttemptBytesResult]{
...
func (ReSpellCastAttempt) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReSpellCastAttempt{}.FindReader(r, cfg, func(_ stream.Match[*ReSpellCastAttemptBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReSpellCastAttempt) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReSpellCastAttemptBytesResult, int64, error) {
	var result *ReSpellCastAttemptBytesResult
	var offset int64
	err := ReSpellCastAttempt{}.FindReader(r, cfg, func(m stream.Match[*ReSpellCastAttemptBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReSpellCastAttempt{}.copyBytesResult(m.Result)
...
func (ReSpellCastAttempt) copyBytesResult(src *ReSpellCastAttemptBytesResult) *ReSpellCastAttemptBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReSpellCastAttemptBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReSpellCastAttempt) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReSpellCastAttemptBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReSpellCastAttempt{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReSpellCastAttempt) processTransform(data []byte, isEOF bool, onMatch func(match *ReSpellCastAttemptBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReSpellCastAttemptBytesResult{}

	for {
		result, ok := ReSpellCastAttempt{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReSpellCastAttempt) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reSpellCastAttemptTransformErrReader{err: err}
	}
	...
	return ReSpellCastAttempt{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReSpellCastAttemptBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReSpellCastAttempt{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reSpellCastAttemptTransformErrReader is a reader that always returns an error.
type reSpellCastAttemptTransformErrReader struct {
	err error
}

func (r *reSpellCastAttemptTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReSpellCastAttempt) getCaptureByIndex(m *ReSpellCastAttemptBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReSpellCastAttempt) SelectReader(r io.Reader, pred func(*ReSpellCastAttemptBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReSpellCastAttempt{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReSpellCastAttempt) processSelect(data []byte, isEOF bool, pred func(*ReSpellCastAttemptBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReSpellCastAttemptBytesResult{}
...
//	})
func (ReSpellCastAttempt) RejectReader(r io.Reader, pred func(*ReSpellCastAttemptBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReSpellCastAttempt{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReSpellCastAttempt) processReject(data []byte, isEOF bool, pred func(*ReSpellCastAttemptBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReSpellCastAttemptBytesResult{}
...
func (r *ReSpellCastAttemptResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReSpellCastAttemptBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReSpellCastAttempt) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReSpellCastAttemptResult

	remaining := input
	offset := 0

	for {
		match, ok := ReSpellCastAttempt{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReSpellCastAttempt) ReplaceAllBytes(input []byte, template string) []byte {
	return ReSpellCastAttempt{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReSpellCastAttempt) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReSpellCastAttemptBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReSpellCastAttempt{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReSpellCastAttempt) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReSpellCastAttemptResult
	match, ok := ReSpellCastAttempt{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReSpellCastAttempt) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReSpellCastAttemptBytesResult
	match, ok := ReSpellCastAttempt{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReSpellCastAttemptReplaceTemplate holds a pre-compiled replace template for the ReSpellCastAttempt pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReSpellCastAttemptReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReSpellCastAttempt) CompileReplaceTemplate(template string) (*ReSpellCastAttemptReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReSpellCastAttemptReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReSpellCastAttemptReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReSpellCastAttemptReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReSpellCastAttemptResult
...
func (t *ReSpellCastAttemptReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReSpellCastAttemptReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReSpellCastAttemptBytesResult
...
func (t *ReSpellCastAttemptReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReSpellCastAttemptResult
	match, ok := ReSpellCastAttempt{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReSpellCastAttemptReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReSpellCastAttemptBytesResult
	match, ok := ReSpellCastAttempt{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
