---
name: azerothcore-scriptai
description: |
  AzerothCore ScriptAI hook system documentation. Covers the Observer pattern,
  creating new ScriptObject types, implementing hooks in ScriptMgr, naming conventions,
  and advanced patterns (filtering via references/bool returns, module-internal hooks).
  Use when: adding hooks to AzerothCore, creating modules, or working with ScriptMgr.
---
...
This guide, together with the [module system](https://www.azerothcore.org/wiki/create-a-module) lets you extend AzerothCore without patching it directly. This allows you to update your repository by keeping your additions and customizations conflict-free!
...
Now you simply call these two functions from anywhere in the core to trigger the event on all registered scripts of that type.

### How to call your hooks

The ScriptMgr class is initialized within AC as a singleton that will contain all the observers (ScriptObjects) and their related registered listeners (hooks).
...
Remember to document your new hook by following the [How to document your code](https://www.azerothcore.org/wiki/how-to-document-code) guide.

When you create a new hook to publish into the AC repo, one of the acceptance criteria is to write proper documentation for it, hence other people know how to use it properly.

### Write a changelog

When you create or modify any hook, you have to create a new changelog to explain to people how to adapt their code and to keep them informed about these changes. Please follow [this guide](https://www.azerothcore.org/wiki/how-to-use-changelog) to learn how to do it.
...
The `[When]` part is optional, but strongly suggested. It helps to understand in which part of the parent function the hook is called.
...
This is the most common one. Basically using the concept of passing a parameter by reference you can change everything that is passed to the hook itself. For instance:
...
You can take a look at [this file](https://github.com/azerothcore/mod-autobalance/blob/master/src/AutoBalance.h) as an example.

> **Note:** You also need to create your own ScriptMgr implementation and offer a singleton to allow calling your hooks.

### Final considerations

There are different other features of the ScriptAI system that have not been included in this documentation, such as the creation of scripts bound to specific entities inside our database (e.g. CreatureScript). This advanced usage can be implemented by replicating the related code we have inside the ScriptMgr files.

## External resources

- [Stack Overflow: Is it possible to turn a core patch into a module for AzerothCore?](https://stackoverflow.com/questions/66340549/is-it-possible-to-turn-a-core-patch-into-a-module-for-azerothcore/66340683#66340683)
