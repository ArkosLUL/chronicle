...
func (w *Watchdog) publishLoop() {
	...
	tkr := w.clock.TickerFunc(w.ctx, periodHeartbeat, func() error {
		...
		if err != nil {
			w.logger.Warn("failed to publish heartbeat on pubsub watchdog", slog.String("error", err.Error()))
		} else {
			w.logger.Debug("published heartbeat on pubsub watchdog")
		}
		return err
	}, "publish")
...
func (w *Watchdog) subscribeMonitor() {
	...
	unsub, err := w.ps.Subscribe(EventPubsubWatchdog, func(context.Context, []byte) {
		w.logger.Debug("got heartbeat for pubsub watchdog")
		select {
		case <-w.ctx.Done():
		case beats <- struct{}{}:
		}
	...
	if err != nil {
		w.logger.Error("watchdog failed to subscribe", slog.String("error", err.Error()))
		close(w.timeout)
		return
	}
	defer unsub()
	for {
		select {
		case <-w.ctx.Done():
			w.logger.Debug("context done; exiting subscribeMonitor")
			return
		case <-beats:
		...
		case <-tmr.C:
			...
			w.logger.Error("pubsub watchdog timeout", slog.String("goroutines", buf.String()))
			close(w.timeout)
			return
		}
	}
