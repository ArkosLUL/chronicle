...
import { getInstanceBackground } from "../utils/instanceImages";
...
export function InstanceDayCard({ instance, showDuration = true }: InstanceDayCardProps) {
  ...
  return (
    <Link to={instanceUrl} className="block">
      <div className="relative h-12 rounded overflow-hidden group cursor-pointer transition-all hover:scale-[1.02] hover:shadow-md">
        ...
        <div className="relative z-10 h-full flex items-center justify-between px-2">
          ...
          {showDuration && duration && (
            <span className="text-[10px] text-white/80 bg-black/40 px-1.5 py-0.5 rounded ml-1 flex-shrink-0">
              {duration}
            </span>
          )}
        </div>
