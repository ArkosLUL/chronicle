...
func (c *cleaner) waitAndClean() {
	...
	if closeErr := rows.Close(); closeErr != nil {
		c.logger.Error("error closing rows", slog.String("error", closeErr.Error()))
	}
	c.logger.Debug("queried names", slog.Any("names", names))
	for _, name := range names {
		_, err := c.db.Exec(fmt.Sprintf("DROP DATABASE IF EXISTS %s", name))
		if err != nil {
			c.logger.Error("error dropping database", slog.String("error", err.Error()), slog.String("name", name))
			return
		}
	...
	c.logger.Debug("finished cleaning")
}
